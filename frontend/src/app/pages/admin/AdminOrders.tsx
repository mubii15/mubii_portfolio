        ...
        <div className="flex flex-col gap-10">
            ...
            <div className="flex justify-between items-end">
                ...
                <div className="flex gap-4">
                    <button className="flex items-center gap-3 px-6 py-3 bg-white/5 border border-white/10 text-white text-[10px] font-black tracking-widest uppercase rounded-sm hover:bg-white/10 transition-all">
                        Export CSV
                    </button>
                    <button className="flex items-center gap-3 px-8 py-3 bg-white text-black text-[10px] font-black tracking-widest uppercase rounded-sm hover:scale-105 transition-all shadow-[0_0_30px_rgba(255,255,255,0.4)]">
                        Inquiry Manager
                    </button>
                </div>
            </div>
            ...
            <div className="flex flex-wrap gap-4 items-center justify-between bg-white/[0.02] border border-white/5 p-4 rounded-[28px]">
                <div className="flex gap-4 flex-1 max-w-md ml-2">
                    <div className="relative flex-1 group">
                        <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-hover:text-white transition-colors" />
                        <input 
                            type="text" 
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            placeholder="SEARCH BY ORDER ID OR CUSTOMER..." 
                            className="w-full bg-black/50 border border-white/5 rounded-md py-3 pl-12 pr-6 text-[10px] font-bold tracking-widest uppercase focus:border-white/50 outline-none transition-all placeholder:text-white/10"
                        />
                    </div>
                </div>
                <div className="flex gap-2">
                    <button className="px-4 py-2 hover:bg-white/5 rounded-lg text-[10px] font-bold tracking-widest uppercase text-white">All Orders</button>
                        ...
                        {MOCK_ORDERS.map((order, i) => (
                            ...
                            >
                                <td className="px-10 py-7">
                                    <span className="text-[11px] font-black text-white tracking-widest group-hover:text-white/70 transition-colors">{order.id}</span>
                                </td>
                                <td className="px-10 py-7">
                                ...
                                <td className="px-10 py-7">
                                    <div className="flex flex-col">
                                        ...
                                        <span className={`text-[8px] font-black uppercase tracking-widest
                                            ${order.type === 'Digital' ? 'text-purple-500' : 'text-white'}
                                        `}>
                                            {order.type} SHIPMENT
                                        </span>
                                    </div>
                                ...
                                <td className="px-10 py-7">
                                    <div className="flex items-center gap-2">
                                        {order.status === 'Completed' && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
                                        ...
                                        <span className={`text-[9px] font-black uppercase tracking-widest
                                            ${order.status === 'Completed' ? 'text-white' : 
...
function SummaryCard({ label, value, change, icon: Icon }: any) {
    return (
        <div className="bg-white/[0.02] border border-white/5 p-8 rounded-sm flex items-center justify-between group hover:border-white/30 transition-all">
            <div className="flex flex-col gap-1">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{label}</span>
                <div className="flex items-baseline gap-3">
                    <span className="text-3xl font-black text-white tracking-tighter">{value}</span>
                    <span className={`text-[10px] font-bold ${change.startsWith('+') ? 'text-white' : 'text-red-500'}`}>{change}</span>
                </div>
            </div>
            <div className="w-12 h-12 bg-white/5 rounded-md flex items-center justify-center text-white group-hover:scale-110 transition-transform">
